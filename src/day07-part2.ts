...
const Joker = 'J';
const CharRanking = ['A', 'K', 'Q', 'T', '9', '8', '7', '6', '5', '4', '3', '2', Joker]
...
class CamelCard {
	...
	constructor(public card: string, public bid: number) {
		...
		const chars = Object.keys(this.#charCount);
		if (chars.includes(Joker)) {
			chars.filter(chr => chr != Joker).forEach(chr => {
				const regexp = new RegExp(Joker, "g");
				this.#card = card.replace(regexp, chr);
				this.value = Math.max(this.value, this.#value);
			})
		}
	}
...
function main() {
	const aoc = new AdventOfCode("day7", false);
	const camelCards = aoc.lines.map(line =>  {
