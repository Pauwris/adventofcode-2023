...
function main() {
    ...
    for (let x=0; x<grid.width - 1; x++) {
        maxValue = Math.max(maxValue, countEnergizedTiles(lines, new GridCoordinate(0, x), 'S'));
        maxValue = Math.max(maxValue, countEnergizedTiles(lines, new GridCoordinate(grid.height-1, x), 'N'));
    }
    for (let y=0; y<grid.height - 1; y++) {
        maxValue = Math.max(maxValue, countEnergizedTiles(lines, new GridCoordinate(y, 0), 'E'));
        maxValue = Math.max(maxValue, countEnergizedTiles(lines, new GridCoordinate(y, grid.width -1), 'W'));
    }

	aoc.printSum(maxValue)
}

function countEnergizedTiles(lines: string[], startingPosition: GridCoordinate, direction: MovementDirection) {
    const grid = new Grid(lines);
    const beams: Beam[] = [];
    const deadBeams: Beam[] = [];
    beams.push(new Beam(grid, startingPosition, direction));
    ...
    return grid.countOfEnergizedTiles;
}
